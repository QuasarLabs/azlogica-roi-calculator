import { COLOMBIAN_PESO } from "./shared";

export const RESULT_LABELS = {
    totalMonthlySavings: { name: "Ahorros totales mensuales", model: "totalMonthlySavings",prefix:COLOMBIAN_PESO },
    systemCost: { name: "Costo del sistema", model: "systemCost",prefix:COLOMBIAN_PESO },
    netMonthlySavings: { name: "Ahorro neto mensual", model: "netMonthlySavings",prefix:COLOMBIAN_PESO },
    totalAnnualSavings: { name: "Ahorros totales anuales", model: "totalAnnualSavings",prefix:COLOMBIAN_PESO },
    annualSystemCost: { name: "Costo anual del sistema", model: "annualSystemCost",prefix:COLOMBIAN_PESO },
    netAnnualSavings: { name: "Ahorro neto anual", model: "netAnnualSavings",prefix:COLOMBIAN_PESO },
    roi: { name: "ROI", model: "roi" },
  };
  